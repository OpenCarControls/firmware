...
*   **ESP (`board-esp`):** Compiled as `rlib`. `esp-rtos` (Embassy executor + time driver), `esp-alloc` (global heap), `esp-backtrace`, and `RUSTFLAGS="-C link-arg=-Tlinkall.x"` all live in `.app_build`, not in `board-esp` itself, because they must be in the final binary crate. The xtask uses `+esp -Zbuild-std=core,alloc` for xtensa targets. Embassy crate versions must match what `esp-rtos` uses (currently `embassy-executor = "0.10.0"`). In 0.10 the spawn API changed: task constructors return `Result<SpawnToken, SpawnError>` and `spawner.spawn()` returns `()` — pattern is `spawner.spawn(task(args).unwrap())`. CAN driver loops run on **core 1** via `esp_rtos::start_second_core` (4-arg form: `cpu_ctrl`, `SoftwareInterrupt::<1>`, `&mut stack`, `closure`); comms and vehicle tasks run on core 0. All ESP HAL dependencies are behind an optional `hardware` Cargo feature; firmware builds (via xtask) enable it automatically. Without the feature, the crate compiles on the host for unit testing (only `embedded-can` and `core-interface` are unconditional). Pure logic that is host-testable (e.g. `compute_mcp_masks`) is kept outside `#[cfg(feature = "hardware")]` blocks. WiFi/MQTT is wired against **`esp-radio 0.18.0`**. There is no shared radio controller — BLE and WiFi initialise independently. BLE: `ble::controller::BleConnector::new(peripherals.BT, ble::Config::default())` (2 args, no `init_radio()` call). WiFi: `esp_radio::wifi::new(peripherals.WIFI, WifiControllerConfig::default())` returns `(controller, interfaces)`; use `interfaces.station` (not `.sta`); configure with `WifiModeConfig::Station(StationConfig::default().with_ssid(...).with_password(...))`. **WiFi can be disabled** via `[network.wifi] enabled = false` in `config.toml`; when disabled, xtask omits the WiFi/MQTT constants and `mqtt_driver_task` spawn from the generated `main.rs`, but `ble_transport_task` is always spawned. With `embassy-net 0.9`, the stack handle is `Stack<'static>` and TCP sockets are created with `TcpSocket::new(*stack, ...)`. In the final binary crate, `esp-rtos` must enable `features = ["embassy", "esp-radio", "esp-alloc"]`; omitting `esp-radio` leads to unresolved `esp_rtos_*` linker errors. **Heap/stack tuning warning:** `esp_alloc::heap_allocator!(size = ...)` and the linker-defined CPU0 stack share the same SRAM budget. If heap is too small, esp-radio/RTOS task creation can panic with `Failed to allocate stack`; if heap is too large, CPU0 can hit stack-guard panics. Keep large BLE/network buffers (for example `HostResources`) in `StaticCell` instead of task locals, and for ESP32-S3 with current task mix prefer around `160 * 1024` heap unless profiling indicates otherwise.

### BLE Implementation Status (ESP)
*   **Host stack / versions:** BLE uses `esp-radio 0.18` + `trouble-host 0.6.0` (with `bt-hci 0.8.1`). Built with features `derive`, `security`, `default-packet-pool-mtu-255`. `trouble_host::new(controller, resources)` returns `Stack<'resources, C, P>` (outer variable); `stack.build()` takes `&'stack self` and returns non-exhaustive `Host { peripheral, runner, .. }` — the `Stack` is still usable after `build()` for `get_bond_information()`, `add_bond_information()`, and `command()`.
...
*   **Stable BLE identity:** Device address is derived from efuse base MAC (`esp_hal::efuse::base_mac_address()` free function returning a `MacAddress` newtype; use `.as_bytes()` to get `&[u8; 6]`) and converted to a static-random BLE address (`mac_to_ble_address`) so identity is stable across reboots.
*   **Advertising mode policy:** `advertise_and_accept()` selects one of two modes on each loop iteration based on `is_pairing_window_open()`:
	*   **Mode A (window open):** `AD_FLAG_LE_LIMITED_DISCOVERABLE | BR_EDR_NOT_SUPPORTED`, full device name, and service UUID in scan response — visible to all BLE scanners. `set_bondable(true)` is applied to accepted connections.
	...
	*   **Controller FAL / Resolving List:** Before each advertise cycle, `update_controller_filter_lists` programs the controller FAL and Resolving List via `stack.command()` with bonded device addresses and IRKs. In Mode B when at least one bond has an IRK (`can_hardware_filter = true`), `AdvFilterPolicy::FilterConn` is set so the hardware rejects connections from unknown devices at the controller level. `FilterConn` is only enabled when the RL has IRK entries so the controller can resolve RPAs before the FAL check — using it without a populated RL would break reconnects from phones using Resolvable Private Addresses.
*   **Security enforcement model:** Both RX and TX GATT characteristics declare `permissions(encrypted)` in the `#[characteristic(...)]` macro (supported in `trouble-host 0.6`). Additional runtime layers:
	*   RX writes rejected with `AttErrorCode::INSUFFICIENT_AUTHORISATION` (British spelling) if peer is not in the paired-phone registry AND the pairing window is closed.
	...
	*   TX notifications withheld until `ble_tx_notify_task` confirms link encryption via `conn.raw().security_level()` (see TX buffering below).
	*   `request_security()` is used to trigger pairing/encryption when needed.
*   **TX buffering during encryption:** `ble_tx_notify_task` holds a `pending_msg: Option<DeviceToApp>` buffer. If a message arrives from `BLE_TX_CHANNEL` before the link is encrypted, it is stored in `pending_msg` (not dropped) and retried every 50 ms. Encryption is checked via a per-connection `tx_auth: AtomicBool` (initialized `false`). On each retry while `tx_auth` is `false`, `ble_tx_notify_task` calls `conn.raw().security_level()` — if the result is `Encrypted`, it sets `tx_auth = true` (cached for all subsequent messages) and sends immediately. This single check covers both the first-pair path (encryption established during `PairingComplete`) and the reconnect path (re-encryption via stored LTK, which never fires `PairingComplete` on iOS). `gatt_event_task` no longer touches `tx_auth`.
*   **Pairing events:** Passkey display/confirm/input and pairing-complete/failure events are handled in the BLE event loop. On `PairingComplete`, new bond data is persisted to flash and added to the controller filter lists. A warning is logged if `security_level` is not encrypted. `tx_auth` is not touched here — TX unblocking is handled entirely by `ble_tx_notify_task` (see TX buffering above).
*   **Known-peer reconnect:** Bonded phones reconnect in Mode B and re-encrypt via LL_ENC_REQ using the stored LTK. `trouble-host` has no dedicated "encryption changed" event, so `ble_tx_notify_task` detects this by polling `conn.raw().security_level()` on each 50 ms retry until it returns `Encrypted`, then sets `tx_auth = true` and begins sending. iOS never sends `Pairing_Request` after LL_ENC_REQ so `PairingComplete` never fires on that path — this is the exact case the `security_level()` check was introduced to handle.
...
```
xtask generates `.app_test_build/` (parallel to `.app_build/`), compiles it with the Xtensa toolchain using `embedded-test 0.7` (features: `external-executor`, `embassy-09`, `xtensa-semihosting`, `panic-handler`) and flashes/runs via `probe-rs run` (invoked with `sudo` because the USB device is `root:root` in the dev container). Test results are reported via semihosting — no defmt or RTT involved. The template (`boards/esp/tests.template.rs`) uses plain `assert!`/`assert_eq!` and includes channel round-trip and `passes_filter` smoke tests. Add custom on-device tests to `boards/esp/tests/hardware.rs` — xtask injects that file automatically.
