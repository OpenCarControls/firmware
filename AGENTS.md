...
| Crate | Tests | Command |
|---|---|---|
| `core-interface` | 52 — `passes_filter`, BLE/MQTT dispatch (platform_id gating, routing), response routing, state publish split, CAN read-only flag, CAN debug streaming (batch filtering, blocklist, dropped-frame counter) | `cargo test -p core-interface -- --test-threads=1` |
