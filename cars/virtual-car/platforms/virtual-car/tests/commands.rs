...
use virtual_car_controller::{
    ...
    proto::{basic_command::Action, BasicCommand, DoorLockCommand},
};
...
fn door_lock_true_succeeds_and_updates_channel() {
    ...
    assert!(result.is_ok());
    let payload = VEHICLE_STATE_CHANNEL.try_receive().expect("no state update after door lock");
    // Verify the state was actually updated by decoding the basic state
    use prost::Message;
    let basic = virtual_car_controller::proto::BasicState::decode(
        payload.basic.as_slice()
    ).unwrap();
...
fn door_lock_false_succeeds_and_updates_channel() {
    ...
    let result = embassy_futures::block_on(process_basic_command(&bytes));
    assert!(result.is_ok());
    let payload = VEHICLE_STATE_CHANNEL.try_receive().expect("no state update after door unlock");
    let basic = virtual_car_controller::proto::BasicState::decode(
        payload.basic.as_slice()
    ).unwrap();
